...
function kinectModule:setLimbFeature2(vec, pos1, pos2)
	vec:setVec3(0, kinectRawToCM(pos1))
	vec:setVec3(3, kinectRawToCM(pos2))
end
...
function kinectModule:extractKinectFeature(data)
	...
	feature:setVec3(limbFeatureSize*4, kinectRawToCM(data:toVector3(3*NUI_SKELETON_POSITION_HIP_CENTER )))
	--offsetFeatureVector(feature, data:toVector3(3*NUI_SKELETON_POSITION_HIP_CENTER ))
	return feature
end

function kinectModule:__init(kinectTracker, mot_table)
	...
	if not mot_table then
		...
		mMot.skin=RE.createVRMLskin(mLoader, true)
	...
	if self.config.filteringMethod=='SDSFilter' then
		self.filter=SDSFilter(mMot.loader, 1)
	else
	...
	if config.debugMode then
		...
		g_skinDebugHuman:setMaterial('lightblue')
	...
	self.integrator=MotionUtil.LoaderToTree(mMot.loader, false, false)
...
function kinectModule:oneStep(iframe)
	...
	--print(mMot.loader:getBoneByTreeIndex(2):getFrame().translation)
...
--			--getPendulum(data:toVector3(i*3),i)
			if state(i+1)==1  or state(i+1) ==2 then
				self.mCON.conPos(i):assign(kinectRawToCM(data:toVector3(i*3)))
			else
				conIndex:pushBack(i)
			end
		end
		...
		for i=1, loader:numBone()-1 do
			dbg.draw("Sphere", loader:getBoneByTreeIndex(i):getFrame().translation*100, "poosi"..i, "red", 2)
		end
		local tempPosevec=vectorn()
		loader:getPoseDOF(tempPosevec)
		g_skinDebugHuman:setPoseDOF(tempPosevec)

		--[[
		print(loader:numBone())
		for i=0, loader:numBone()-1 do
			print(loader:bone(i))
		end
		]]
...
function kinectModule:solveIK(sourcePose, state, data, featureK)
	...
	for limb=1,4 do
		...
		if state(ee+1)~=STATE_FAILED then
			--addConstraints
			--footPos(i):assign((data:toVector3(ee*3)*config.kinectScale+config.kinectPosOffset) /config.skinScale)
	...
	for i,v in ipairs(elbows) do
		...
		local goal_elb=(kinectRawToCM(data:toVector3(v*3)))/config.skinScale
...
function kinectModule:stepKinematic_row( pose, dpose)
	...
	self.integrator:setPoseDOF(dofInfo,pose)
	self.integrator:integrate(dofInfo, dpose, 1/120)
	self.integrator:getPoseDOF(dofInfo, pose)
...
function SDSFilter:__init(loader, mass, pose)
	self.mPendulum={}
	-- the smaller, the smoother
	self.scaleFactor=vectorn(loader.dofInfo:numDOF()+1)
	self.scaleFactor:setAllValue(1)
   -- root position
   self.scaleFactor:range(0,3):setAllValue(config.skinScale/100) -- change to meter scale
   -- root orientation
